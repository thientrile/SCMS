...
const authertication = async (req, res, next) => {
	try {
		...
		return JWT.verify(accessToken, keyStore.tkn_publicKey, (err, decoded) => {
			if (err) {
				throw new AuthFailureError(' Token has expired');
			}
